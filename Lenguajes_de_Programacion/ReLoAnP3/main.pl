...
% frec_absoluta(+Xs,?Fs)
%
% Dada una lista Xs, devuelve en Fs una lista
% de pares (X,N) donde X es un elemento de Xs 
% y N el numero de veces que X aparece en Xs
% (es decir, su frecuencia absoluta)
frec_absoluta(Xs,Fs) :-
	frec_absoluta(Xs,[],Fs).

frec_absoluta([],Ac,Ac).
frec_absoluta([X|Xs],Ac,Fs) :-
	agrega(X,Ac,NAc),
	frec_absoluta(Xs,NAc,Fs).
	
% agrega(+X,+Fs,?FXs)
%
% agrega una X a la tabla de frecuencias Fs,
% obteniendo FXs.
agrega(X,[],[(X,1)]).
agrega(X,[(X,N)|Fs],[(X,N1)|Fs]) :-
	N1 is N + 1.
agrega(X,[(Y,N)|Fs],[(Y,N)|FXs]) :-
	X \= Y,
	agrega(X,Fs,FXs).
...
% maximo(+Fs,?P)
%
% Dada una tabla de frecuencias absolutas Fs,
% devuelve el par P con la maxima frecuencia
maximo([P],P).
maximo([(X,N),(Y,M)|Xs],Max) :-
	max_par((X,N),(Y,M),T),
	maximo([T|Xs],Max).

% maximo(+X,+Y,M)
%
% Dados dos pares X e Y, devuelve en P el que
% tenga la mayor frecuencia absoluta 
max_par((X,N),(_,M),(X,N)) :-
	N >= M.
max_par((_,N),(Y,M),(Y,M)) :-
	N < M.
...
% Prueba de Moda
moda(Fs, Moda, N):-
	frec_absoluta([kevin_spacey,wes_bentley,chris_cooper,kevin_spacey,chris_cooper], Fs),
	maximo(Fs,(Moda,N)).

/***************************************************************************************/
/* 1. Encontrar dos actores o actrices que han trabajado juntos en mas de una película */
/***************************************************************************************/
findActorOrActrizTogetherInSameMovie(A1,A2,M1,M2):-
	movie(M1,_),
	movie(M2,_),
	actorOrActriz(M1,A1),
	actorOrActriz(M2,A2),
...
/**************************************************************************************/
/* 2. Encontrar todos los actores/actrices que hayan trabajado para un mismo director */
/**************************************************************************************/

findActorOrActrizWorkedForDirector(Autores):-
	findall(res(A,D), trabajonParaDirector(A,D), Autores).
	

/******************************************************************/
/* 3. Encontrar el actor o actriz con mas actuaciones en un año. */
/*****************************************************************/

masActuaciones(AutorModa, Y):-
	findall(A, actorEnAnio(A,Y), Autores),
	frec_absoluta(Autores, Fs),
	maximo(Fs,(AutorModa,N)).
	

/******************************************************************************/
/* 4. Encontrar el director que mantiene al “mismo cuadro”, es decir el mismo */
/* grupo de actores/actrices entre dos películas distintas                    */
/******************************************************************************/
...
mismoCuadro(M1,M2, D):-
	...
	director(M2,D),
	M1 \= M2,
	findall(A1,actorOrActriz(M1,A1),Aut1),
	findall(A2,actorOrActriz(M2,A2),Aut2),
	Aut1 == Aut2.
	
/*********************************************************************************/
/* 5. N grados de separación. Existe la llamada “Ley de Bacon” o los seis grados */
/* de separación de Kevin Bacon donde es posible unir a cualquier actor con      */
/* Kevin Bacon a traves de menos de seis peliculas en que hayan trabajado        */
/* juntos. Escriba un predicado para conectar en N peliculas a dos actores o     */
/* actrices                                                                      */
/*********************************************************************************/
